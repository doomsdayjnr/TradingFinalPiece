...
bool TFP_ValidateLicenseOnline()
{
   ...
   if(status == -1)
   {
      ...
      TFP_LastMessage = "WebRequest failed. In MT4, enable WebRequest and add your Trading Final Piece domain. Error: " + IntegerToString(errorCode);
      TFP_LastAllowed = false;
      return false;
   }
...
void TFP_ShowUnauthorizedMessage()
{
   string message =
      ...
      "--------------------------------------------------\n"
      " Account #" + IntegerToString(AccountNumber()) + " is not verified under our partner link.\n\n"
      " HOW TO UNLOCK THIS EA:\n"
      " 1. Visit: https://your-domain.com/dashboard\n"
      " 2. Register/Link your MT4 or MT5 account under our IB code.\n"
      " 3. Submit Account #" + IntegerToString(AccountNumber()) + " on your dashboard.\n\n"
      " " + TFP_LastMessage + "\n"
      "--------------------------------------------------";
